...
public class Main extends InputAdapter implements ApplicationListener {
    ...
    private void updateKoala (float deltaTime) {
        ...
        // Umbral de muerte (puedes ajustarlo según el nivel)
        float deathThreshold = -10f;

        // Verifica si el koala ha caído por debajo del umbral
        if (koala.position.y < deathThreshold) {
            koala.position.set(20, 20);  // Reinicia la posición del koala
            koala.velocity.set(0, 0);    // Resetea la velocidad
            koala.state = Koala.State.Standing; // Asegura que el koala esté de pie
            koala.stateTime = 0; // Reinicia el tiempo de la animación

            // Lógica adicional: Mostrar mensaje de muerte o reiniciar el nivel
            System.out.println("¡El Koala ha muerto!");
            // Aquí podrías recargar el mapa, reiniciar la escena, etc.
        }
